...
<Update label="September 12, 2026" description="" tags={["Blacklists", "Custom Rules", "Experience"]}>
#### 1. Smarter Blacklist Management

Blacklists are easier to manage day to day. Turn any blacklist on or off without deleting it, **edit existing entries**, and use **bulk actions** to delete or move multiple items to another list of the same type. Blacklist detail pages also include breadcrumbs so you can navigate back quickly.

#### 2. Enable or Disable Custom Rules

Custom rules now have an on/off toggle in the table, so you can pause a rule temporarily without removing it.

#### 3. Event Details in a Drawer

Clicking an event now opens its details in a side drawer instead of a modal, making it easier to review events while keeping context on the events page. Event type icons have also been corrected for clearer identification.

</Update>

<Update label="September 2026" description="" tags={["Experience"]}>
#### 1. Refreshed Dashboard Design

We've redesigned the Greip Dashboard with a cleaner, more consistent interface across every page. Navigation, spacing, typography, and colors are now unified so the product feels faster to scan and easier to use in both light and dark mode.

#### 2. Improved Sidebar & Mobile Experience

The sidebar has been rebuilt for clearer section grouping, smoother scrolling, and better behavior on smaller screens. Mobile layouts now adapt more reliably, with drawers and panels that stay out of the way until you need them.

#### 3. Unified Modals, Drawers & Dialogs

Modals, drawers, and confirmation dialogs now share the same layout and styling. Event details, filters, custom rules, integrations, and account settings all open in consistent surfaces that match the rest of the dashboard.

#### 4. System Theme Support

The dashboard now follows your system light or dark preference automatically, with refined dark-mode contrast and form controls across tables, charts, settings, and billing pages.

#### 5. Updated Home & Auth Experience

The home page has a refreshed layout with clearer quick links and an insights overview at the top. Sign-in, registration, and password flows also received a visual refresh with tighter layouts and improved readability.
...
<Update label="September 2024" description="" tags={["Experience", "Invoicing", "Integrations", "Authentication"]}>
#### 1. Design Updates
